...
qc_primes, groups, good_primes := 
               find_qc_primes(X : mws_primes_bound := 20000, qc_primes_bound := 100, number_of_bad_disks := 0, inf_modp_allowed := false,
                                   ordinary := false, known_rat_pts := ptsX, printlevel := 0);
printf "You might want to use the primes %o for QC.\n", qc_primes;
// [ 3, 5, 11, 17, 23, 41, 61, 71, 89 ]
// < 17: problems
// 17: 2 cosets remaining after v=4783
// 23: Hecke operator does not generate
// 41: Runtime error in 'PointsAtInfinity'
// 61: 60 cosets remaining after v=3943
// 71: 4 cosets remaining after v=359
// 89: no cosets eliminated
// [17, 61, 71]: 8 cosets remaining after v=94009
  ...
  try
    ...
    good_affine_rat_pts_xy, no_fake_pts, bad_affine_rat_pts_xy, data, fake_rat_pts, bad_Qppoints :=
        QCModAffine(y^2-f, p : printlevel := 1, // unit_root_splitting := true,
              N := N1, prec := 40, base_point := base_pt);
...
// there are not bad disks
/*
// Finally exclude points in bad disks and infinite. We checked above that none of these contain a known rational point.
printf "Use the Mordell-Weil sieve to show that there are no rational points in bad or infinite disks\n";
